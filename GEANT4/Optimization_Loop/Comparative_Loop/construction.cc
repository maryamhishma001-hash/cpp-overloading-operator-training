...
: logicTarget(nullptr), logicModerator(nullptr), logicFastFilter(nullptr), logicGammaFilter(nullptr),  
  ...
  fModeratorThickness(32.0 * cm),
  fReflectorThickness(20.0 * cm),
  fFastFilterThickness(0.4 * cm),
  fGammaFilterThickness(2.5 * cm),
  fCollimatorLength(3.0 * cm)
{
...
G4VPhysicalVolume *MyDetectorConstruction::Construct()
{
   ...
   G4double hzFastFilter = fFastFilterThickness / 2.0;
   G4double hzGammaFilter = fGammaFilterThickness / 2.0;
   G4double hzCollimator = fCollimatorLength / 2.0;
   G4double reflectorThickness = fReflectorThickness;
...
void MyDetectorConstruction::SetFastFilterThickness(G4double val)
{
    fFastFilterThickness = val;
    G4RunManager::GetRunManager()->GeometryHasBeenModified();
}

void MyDetectorConstruction::SetGammaFilterThickness(G4double val)
{
    fGammaFilterThickness = val;
    G4RunManager::GetRunManager()->GeometryHasBeenModified();
}

void MyDetectorConstruction::SetCollimatorLength(G4double val)
{
    fCollimatorLength = val;
    G4RunManager::GetRunManager()->GeometryHasBeenModified();
}

void MyDetectorConstruction::SetReflectorThickness(G4double val)
{
    fReflectorThickness = val;
    G4RunManager::GetRunManager()->GeometryHasBeenModified();
}
