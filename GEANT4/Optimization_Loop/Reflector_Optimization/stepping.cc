...
void MySteppingAction::UserSteppingAction(const G4Step* step)
{
    ...
    if ((volName == "logicTarget" || volName == "Target") && particleName == "proton")
    {
        ...
        man->FillNtupleDColumn(0, 1, preStepPoint->GetTotalEnergy() / keV);
        man->FillNtupleDColumn(0, 2, postStepPoint->GetTotalEnergy() / keV);
        man->FillNtupleDColumn(0, 3, pos.x() / m);
        man->FillNtupleDColumn(0, 4, pos.y() / m);
        man->FillNtupleDColumn(0, 5, pos.z() / m);
        man->AddNtupleRow(0);
    }
    ...
    2.53e-11, 2.59e-11, 2.65e-11, 2.70e-11, 2.76e-11, 2.81e-11, 2.86e-11, 2.91e-11, 2.95e-11};
                    ...
                    G4double x_cm = track->GetPosition().x() / cm;
                    G4double y_cm = track->GetPosition().y() / cm;
    ...
    1.21e-11, 1.41e-11, 1.61e-11, 2.01e-11, 2.40e-11};
                    ...
                    man->FillNtupleDColumn(2, 2, track->GetPosition().x() / cm);
                    man->FillNtupleDColumn(2, 3, track->GetPosition().y() / cm);
