...
void MyRunAction::EndOfRunAction(const G4Run* aRun)
{
    ...
    if (G4Threading::IsMasterThread()) 
    {
        ...
        std::ifstream checkFile("reflector_sweep_summary.csv");
        bool isEmpty = !checkFile.is_open() || checkFile.peek() == std::ifstream::traits_type::eof();
        checkFile.close();

        std::ofstream summaryFile("reflector_sweep_summary.csv", std::ios::app);
        
        if (isEmpty) {
            summaryFile << "ReflectorThickness_cm,EpiFlux_n_cm2_s,ThermalToEpithermalRatio,D_Fast_epi,D_Gamma_epi,Directionality\n";
        }

        auto detectorConstruction = static_cast<const MyDetectorConstruction*>(
            G4RunManager::GetRunManager()->GetUserDetectorConstruction()
        );
        G4double reflectorThick_cm = detectorConstruction ? (detectorConstruction->GetReflectorThickness() / CLHEP::cm) : 0.0;

        summaryFile << reflectorThick_cm << ","
                    << realFluxEpithermal << ","
